...
cridCopyButton.onclick = async () => {
  if (navigator.clipboard) {
    ...
    //Lmao, I didn't think a fallback will work.
...
async function Register() {
  ...
  try {
    const genPassword = `${randomUUID().slice(0, 5)}-${randomUUID().slice(
      0,
      5
    )}-${randomUUID().slice(0, 5)}-${randomUUID().slice(0, 5)}`.toUpperCase();
    let oldUsernames = await DDB.get("usernames");
    if (oldUsernames === null) {
      oldUsernames = ["remohexa", "preview-user"];
      await DDB.set("usernames", oldUsernames);
    }
    if (!oldUsernames.includes(usernamex)) {
      const newMatrixID = await DDB.getNewUserId();
      let defImgHex;
      if (fromHex(newMatrixID) > 40) {
        defImgHex = toHex(Math.floor(Math.random() * 39) + 1);
      } else {
        defImgHex = newMatrixID;
      }
      await DDB.set("account", {
        username: usernamex,
        matrix_id: newMatrixID,
        matrix_key: genPassword,
        pfp_url: `/assets/static/pfps/p${defImgHex}`,
        default_pfp: `/assets/static/pfps/p${defImgHex}`,
        ig: "",
        tg: "",
        dc: "",
      });
      cridMatrixId.value = newMatrixID;
      cridMatrixKey.value = genPassword;

      postRegMatrixId = newMatrixID;
      defaultImgUrl = `/assets/static/pfps/p${defImgHex}`;
...
_saveButton.onclick = async () => {
  ...
  const ig = `${_accountIg.value}`;
  const tg = `${_accountTg.value}`;
  ...
  oldAccount.dc = _accountDc.value;
