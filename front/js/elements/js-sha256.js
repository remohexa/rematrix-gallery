...
export const sha256 = window.sha256;
export const sha224 = window.sha224;
delete window.sha256;
delete window.sha224;
export async function getFFileHash(file, algorithm = "SHA-256") {
  const buffer = await file.arrayBuffer();
  const uint8Array = new Uint8Array(buffer);

  if (algorithm.toUpperCase() === "SHA-256") {
    return sha256(uint8Array);
  } else if (algorithm.toUpperCase() === "SHA-224") {
    return sha224(uint8Array);
  } else {
